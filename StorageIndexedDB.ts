    ...
    this.ready = new Promise<IIndexedDBOpenResponse>((success: OpenResponseSuccess, reject) => {
      ...
      dbRequest.onupgradeneeded = () => {
        ...
        try {
          ...
          const newIndexNames = otherIndexNames.filter(
          ...
          alert(`The storage database needs to be upgraded.
...
The page will now refresh!`);
      ...
      dbRequest.onerror = event => {
        const msg =
          dbRequest.error === null ? "IndexedDB dbRequest error" : dbRequest.error.message;
        reject({ msg, event });
      };
      ...
      this.ready.then(response => {
        ...
        try {
          ...
          cursorRequest.onsuccess = () => {
            ...
            if (cursor !== null && (count === undefined || c < count)) {
          ...
          cursorRequest.onerror = event => {
            const msg = cursorRequest.error === null ? "Cursor Error" : cursorRequest.error.message;
            reject({ msg, event });
          };
